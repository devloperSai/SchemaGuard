...
import type { DriftLog, ChangeType } from "@/lib/mock";
...
function DriftHistory({ endpointId }: { endpointId: string }) {
  const logs = useStore((s) => s.driftLogs.filter((d) => d.endpointId === endpointId));
      ...
      {open && (
        <div className="space-y-4 p-4">
          ...
          {logs.map((d) => (
            <article
              key={d.id}
              className="overflow-hidden rounded-lg border border-border/60 bg-surface-2/40"
            >
                    ...
                    {d.diff.map((row, i) => (
                      <tr key={i} className="border-b border-border/40 last:border-0">
                        <td className="px-4 py-2.5">
                          <ChangePill type={row.changeType} />
                        </td>
                        <td className="text-mono px-4 py-2.5 text-xs">{row.field}</td>
                        <td className="text-mono px-4 py-2.5 text-xs text-muted-foreground">
                          {row.baselineType}
                        </td>
                        <td className="text-mono px-4 py-2.5 text-xs text-foreground">
                          {row.liveType}
                        </td>
                      </tr>
                    ))}
...
function ChangePill({ type }: { type: ChangeType }) {
  const cfg = {
    ...
    TYPE_CHANGED: {
      c: "text-amber-300 bg-amber-500/10 border-amber-500/20",
      I: ArrowRightLeft,
      label: "TYPE CHANGED",
    },
  }[type];
  ...
  return (
    <span
      className={`text-mono inline-flex h-5 items-center gap-1 rounded border px-1.5 text-[10px] font-semibold tracking-wider ${cfg.c}`}
    >
