...
export function MonitoringWizard({
  ...
  const handleSave = () => {
    setSaving(true);
    // Tags stay as human-readable metadata for the sidebar/UI, but the ACTUAL
    // check config is now written onto the Endpoint document itself so the
    // poller can enforce it — previously these were the only place this data
    // was recorded, and proxyService never read them.
    const tags: string[] = [
      "monitored",
    ...
    const ep: EndpointConfig = {
      ...
      // ── Actually persist what the wizard collected ──
      checkTypes: form.checkTypes,
      expectedStatus:
        form.checkTypes.includes("uptime") && form.expectedStatus
          ? Number(form.expectedStatus) || null
          : null,
      responseTimeThresholdMs: form.checkTypes.includes("response_time")
        ? form.responseTimeThresholdMs
        : null,
      contentMatch: form.checkTypes.includes("content_match")
        ? { field: form.contentMatchField, value: form.contentMatchValue }
        : { field: "", value: "" },
