...
import { Plus, X, ChevronDown, Shield } from "lucide-react";
...
export function RequestTabsBar() {
  ...
  if (tabs.length === 0) {
    return (
      <>
        <div className="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 bg-surface-2/30 px-2">
          {/* Monitor — first, primary */}
          <button
            onClick={() => setMonitoringOpen(true)}
            className="inline-flex h-7 items-center gap-1.5 rounded-md border border-brand/30 bg-brand/10 px-2.5 text-xs font-semibold text-brand transition-all hover:bg-brand/20 hover:border-brand/50"
          >
